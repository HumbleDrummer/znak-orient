...
The application imports a JSON evidence package, normalizes structured observations, rejects duplicates and stale or unauthorized changes, requires subject-scoped value-bound validation for facts, preserves incompatible claims as disputes, verifies or falls back from checkpoints, and produces one canonical orientation result. A responsive local UI shows Noise Intake, Current Position, Conflict and Unknowns, Recovery Card, Source Evidence, and Validation Receipt. A small animated orientation guide reacts to voltage and repeats the exact selected next step without inventing another one.
...
- minimum-sufficient recovery without pre-checkpoint history;
