...
1. Read the authorized build PIN and separated historical claims from current proof.
2. Audited candidate directories and Git markers before writing code.
3. Classified the experimental `znak-orientation-lab` repo and unversioned `znak_orient_mvp` directory as separate boundaries.
4. Created an isolated Git repository with dated, scoped commits.
5. Implemented the deterministic reducer, judge-safe fixture, tests, loopback API, and responsive interface.
6. Executed automated tests and the CLI transform.
7. Verified the real UI in the Codex in-app browser and used local headless Chromium only when its viewport override proved unreliable.
8. Added the requested code-native animated guide and verified its state binding, exact-step cue, mobile layout, and reduced-motion behavior.
9. Retained desktop/mobile screenshots, a 35.12-second local workflow capture, a machine-readable browser QA receipt, a fidelity ledger, clean-checkout evidence, and submission drafts.
